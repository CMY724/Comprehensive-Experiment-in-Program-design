...
//在字符串 id 里查找字符串 txt 第一次出现的位置。
int index_KMP(char* txt, char* id, int id_size, int txt_size)
{
    if (txt_size == 0) return -1;

    int* next = (int*)malloc(sizeof(int) * txt_size);
    ...
    free(next);
    return -1;
}//当匹配失败时，不要让模式串从头重新开始，而是利用已经匹配成功的信息，直接跳到一个合理的位置继续匹配。
//搜索id是不是在txt中，返回第一次出现的位置
